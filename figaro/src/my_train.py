...
from models.hidden_mapper import ConditionalSeq2Seq
...
def main():
    ...
    # call the setup function, which would be hiddenly called in pt_lightninh
    datamodule.setup()
    my_model = ConditionalSeq2Seq(model)

    optim = torch.optim.Adam(model.parameters())

    train_one_epoch(datamodule.train_dataloader(), my_model, optim=optim)


def train_one_epoch(dl, model, optim):
